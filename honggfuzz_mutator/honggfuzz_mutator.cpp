...
//The following mangle methods are taken from honggfuzz, see:
...
static int input_parseDictionary(honggfuzz_state_t * state) {
	...
	for (;;) {
		...
		str->next = state->dictq;
		state->dictq = str;
		state->dictionary_count += 1;
	}
...
static honggfuzz_state_t * setup_options(char * options)
{
	...
	PARSE_OPTION_STRING(state, options, dictionary_file, "dictionary_file", FUNCNAME(cleanup));
...
{
	string_t * next, *current;
	for (current = honggfuzz_state->dictq; current; current = next)
	{
		next = current->next;
		free(current->s);
		free(current);
...
{
	honggfuzz_state_t * honggfuzz_state = (honggfuzz_state_t *)mutator_state;
	json_t *state_obj, *iteration_obj, *random_state0, *random_state1, *dictionary_file, 
		*temp, *dictionary_list, *dictionary_obj;
	string_t * current;
	char * ret;

	state_obj = json_object();
	iteration_obj = json_integer(honggfuzz_state->iteration);
	random_state0 = json_integer(honggfuzz_state->random_state[0]);
	random_state1 = json_integer(honggfuzz_state->random_state[1]);
	if (!state_obj || !iteration_obj || !random_state0 || !random_state1)
		return NULL;

	json_object_set_new(state_obj, "iteration", iteration_obj);
	json_object_set_new(state_obj, "random_state0", random_state0);
	json_object_set_new(state_obj, "random_state1", random_state1);
	if (honggfuzz_state->dictionary_file)
	{
		dictionary_file = json_string(honggfuzz_state->dictionary_file);
		if(!dictionary_file)
			return NULL;
		json_object_set_new(state_obj, "dictionary_file", dictionary_file);

		//Add the dictionary list to the json object
		dictionary_list = json_array();
		if (!dictionary_list)
			return NULL;
		for (current = honggfuzz_state->dictq; current; current = current->next)
		{
			dictionary_obj = json_object();
			if (!dictionary_obj)
				return NULL;
			ADD_STRING(temp, current->s, dictionary_obj, "s");
			ADD_INT(temp, current->len, dictionary_obj, "len");
			json_array_append_new(dictionary_list, dictionary_obj);
		}
		json_object_set_new(state_obj, "dictionary", dictionary_list);
	}
...
HONGGFUZZ_MUTATOR_API int FUNCNAME(set_state)(void * mutator_state, char * state)
{
	...
	get_item(state, honggfuzz_state->iteration, temp_int, get_int_options, "iteration", result);
	get_item(state, honggfuzz_state->random_state[0], temp_uint64t, get_uint64t_options, "random_state0", result);
	get_item(state, honggfuzz_state->random_state[1], temp_uint64t, get_uint64t_options, "random_state1", result);
	...
	if (result > 0)
	{
		...
		FOREACH_OBJECT_JSON_ARRAY_ITEM_BEGIN(state, dictionary, "dictionary", dictionary_obj, result)
			...
			dictionary_item = (string_t *)malloc(sizeof(dictionary_item));
			get_item(dictionary_obj, dictionary_item->s, temp_str, get_string_options_from_json, "s", inner_result);
			...
			dictionary_item->next = honggfuzz_state->dictq;
			honggfuzz_state->dictq = dictionary_item;
...
{
	honggfuzz_state_t * honggfuzz_state = (honggfuzz_state_t *)mutator_state;
	return honggfuzz_state->iteration;
}

/**
 * This function will return the total possible number of mutations with this
 * mutator.  For this mutator, there are an infinite number of mutations.
 * @param mutator_state - a mutator specific structure previously created by the create function.
 * @return the number of possible mutations with this mutator, or -1 for infinite.
 */
...
HONGGFUZZ_MUTATOR_API int FUNCNAME(set_input)(void * mutator_state, char * new_input, size_t input_length)
{
	...
	honggfuzz_state->input = (char *)malloc(input_length);
	if (!honggfuzz_state->input) { //malloc failed
		return -1;
	}
	honggfuzz_state->input_length = input_length;
	memcpy(honggfuzz_state->input, new_input, input_length);
	return 0;
}
