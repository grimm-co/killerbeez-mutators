...
AFL_MUTATOR_API int FUNCNAME(help)(char ** help_str)
{
	GENERIC_MUTATOR_HELP(
		...
		"\thavoc_div             A divisor for determining the number of rounds that the havoc stage should run (typically 1, 2, 5, or 10)\n"
		"\tperf_score            A performance score used to determine how long to run the havoc and splice stages.  Typically 100, higher\n"
		"\t                      results in a larger number of mutations in these stages before moving on.\n"
		"\tqueue_cycle           The queue round counter.  Used in determining how to mutate input.  Generally this shouldn't need to be set\n"
		...
		"\tsplice_filenames      An array of files to use during afl's splice stage, for mixing with the input\n"
		"\n"
	);
}
